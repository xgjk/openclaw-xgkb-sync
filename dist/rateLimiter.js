...
class RateLimiter {
    ...
    async acquire() {
        ...
        if (cooldownWait > 0) {
            console.warn(`[${this.label}] 限流冷却中，等待 ${Math.ceil(cooldownWait / 1000)}s ` +
        ...
        if (waitMs >= 1_500) {
            console.warn(`[${this.label}] 令牌不足，等待 ${Math.ceil(waitMs / 1000)}s` +
    ...
    onRateLimited(retryAfterMs) {
        ...
        if (until > this.pauseUntil) {
            ...
            console.warn(`[${this.label}] 触发限流，暂停 ${Math.ceil(pauseMs / 1000)}s` +
                `（恢复约 ${new Date(this.pauseUntil).toLocaleTimeString('zh-CN')}）`);
        }
        // 令牌清零，防止冷却结束后立即打满一批
        this.tokens = 0;
