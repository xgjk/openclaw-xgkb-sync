...
exports.MassSyncProtectionError = void 0;
exports.evaluateMassSyncProtection = evaluateMassSyncProtection;
exports.isMassSyncProtectionError = isMassSyncProtectionError;
const MAX_SAFETY_PATH_SAMPLES = 20;
/**
 * 在任何批量上传/下载发生前执行的纯判断。
 * 阈值表示“允许的最大数量”，因此只有严格超过时才熔断。
 */
function evaluateMassSyncProtection(input) {
    if (!input.enabled)
        return null;
    ...
    if (!uploadExceeded && !downloadExceeded)
        return null;
    const directions = [];
    if (uploadExceeded)
        directions.push(`上传 ${uploadCount} > ${input.maxUploads}`);
    if (downloadExceeded)
        directions.push(`下载 ${downloadCount} > ${input.maxDownloads}`);
    const samplePaths = [
    ...
    return {
        ...
        samplePaths,
        reason: `大批量同步保护触发（${directions.join('，')}；` +
...
class MassSyncProtectionError extends Error {
    trip;
    constructor(trip) {
        super(trip.reason);
...
exports.MassSyncProtectionError = MassSyncProtectionError;
function isMassSyncProtectionError(value) {
    return value instanceof MassSyncProtectionError;
}
//# sourceMappingURL=syncSafety.js.map