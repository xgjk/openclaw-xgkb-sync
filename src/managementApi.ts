...
export class ManagementApi {
  ...
  private handleSyncAll(res: http.ServerResponse): void {
    const scheduler = this.opts.getScheduler();
    const config = scheduler.getConfig();
    const enabled = config.mappings.filter((m) => isMappingEffectiveEnabled(m, config.mappings));
  ...
  private handleSyncOne(res: http.ServerResponse, mappingId: string): void {
    ...
    if (!isMappingEffectiveEnabled(mapping, config.mappings)) {
  ...
  private handleListMappings(res: http.ServerResponse): void {
    ...
    const duplicateGroups = findDuplicateLocalRootGroups(fileMappings);
    ...
    this.sendJson(res, 200, {
      ...
      mappings: fileMappings.map((m) => ({
        ...
        syncEffective: isMappingEffectiveEnabled(m, fileMappings),
  ...
  private async handleCreateMapping(
    ...
    try {
      existingIds = readMappingsFromConfigFile(this.opts.configPath).map((m) => m.mappingId);
    } catch {
    ...
    const downgraded = downgradeMappingIfLocalRootConflict(mapping, [...fileMappings, mapping]);
  ...
  private async handleUpsertMapping(
    ...
    try {
      const fileMappings = readMappingsFromConfigFile(this.opts.configPath);
      ...
      if (idx === -1) {
        ...
        const downgraded = downgradeMappingIfLocalRootConflict(mapping, [...fileMappings, mapping]);
        mapping = downgraded.mapping;
        if (downgraded.warning) saveWarnings = [downgraded.warning];
      } else {
        ...
        next[idx] = mapping;
        const downgraded = downgradeMappingIfLocalRootConflict(mapping, next);
        mapping = downgraded.mapping;
        if (downgraded.warning) saveWarnings = [downgraded.warning];
      }
    } catch (e) {
    ...
    if (created) {
      ...
      if (mapping.enabled && isMappingEffectiveEnabled(mapping, reloadResult.config.mappings)) {
  ...
  private async handleSetMappingEnabled(
    ...
    let mapping: SyncMapping = { ...existing, enabled };
    let saveWarnings: string[] = [];
    if (enabled) {
      const next = [...fileMappings];
      next[idx] = mapping;
    ...
    if (!reloadResult.ok) {
      console.warn(
        `[ManagementApi] mapping "${mappingId}" 已${actionLabel}但热重载失败: ${reloadResult.error}`,
      );
      return this.sendJson(res, 200, {
        ok: true,
        reloadOk: false,
