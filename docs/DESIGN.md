...
| **单文件隔离** | 普通文件错误独立 try/catch，不影响本轮其他文件 |
| **防重入锁** | 同一 mappingId 同时最多一个同步协程，新触发合并为 pending |
| **指数退避** | 5xx 网络错误重试，退避 1s/2s，最多 3 次 |
| **永久错误熔断** | 上传先串行到一次真实成功再放开并发；鉴权、权限或远端初始化参数类拒绝后停止剩余远端操作。单文件参数错误保持隔离。mapping 冷却 30 分钟起指数增长至 24 小时并持久化，手动同步可随时探测 |
| **macOS watcher 资源边界** | 非 polling 模式使用原生递归 `fs.watch`，父子 root 合并，最多 256 个 root；超出部分由定时 sync 兜底 |
