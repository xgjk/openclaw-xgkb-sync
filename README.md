...
上传队列先串行探测到一次真实成功，之后才放开配置并发。明确发生在远端写操作上的权限拒绝会停止本轮剩余写入，持久化为临时写能力限制：`bidirectional` 实际按 `pull` 运行，`push` 暂停。本地待上传内容保留，服务按 30 分钟、2 小时、6 小时、24 小时的有上限退避自动复测。只有真实远端写 API 成功才恢复配置方向。读取、认证和初始化类永久错误仍使用原有熔断逻辑。
